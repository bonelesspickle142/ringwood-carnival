...
function SponsorCard({ sponsor, large }) {
  ...
  return (
    <Wrapper
      {...props}
      className={`bg-card border border-border rounded-2xl p-6 flex flex-col items-center justify-center gap-3 transition-colors ${sponsor.url ? "hover:border-primary/40 cursor-pointer" : ""} ${large ? "min-h-[170px]" : "min-h-[130px]"}`}
    >
      {sponsor.image_url ? (
        <img
    ...
    <div className="min-h-screen">
      ...
      <div className="px-5 md:px-12 py-6">
        <div className="bg-card border border-border rounded-2xl p-6">
          ...
          <p className="text-foreground text-sm leading-relaxed mt-3">
            We're proud to announce that <strong className="text-secondary">Economy Hire</strong> is our headline sponsor for this year. A huge thank you to them for leading the way, and to all our wonderful sponsors listed below.
          </p>
        </div>
      </div>
      ...
      {headline.length > 0 && (
        <div className="px-5 md:px-12 py-4">
          <div className="text-center mb-4">
            <span className="inline-flex items-center gap-1.5 bg-secondary/10 text-secondary text-xs font-heading font-bold uppercase tracking-wide px-3 py-1 rounded-full">
      ...
      <div className="px-5 md:px-12 py-6 pb-32">
        {loading ? (
          ...
          <>
            <h2 className="font-heading font-bold text-foreground text-lg mb-4">With Thanks To</h2>
          ...
          <div className="text-center py-20 bg-card rounded-xl border border-border">
            <p className="text-muted-foreground text-lg mb-2">Sponsor details coming soon!</p>
            <p className="text-muted-foreground/60 text-sm">Check back for our full list of sponsors</p>
          </div>
        ) : null}
      </div>
    </div>
